...
### 5.9. Google Calendar Sync Formatter

El mòdul `googleCalendarSyncFormatter.ts` s'encarrega de formatar les dades dels esdeveniments per a la seva sincronització amb Google Calendar. Aquest mòdul centralitza tota la lògica de formatatge per garantir la consistència de les dades enviades al calendari.

#### Funcionalitats principals:

1. **Format de Dates**
   - `formatSimpleDMY`: Converteix una data ISO (YYYY-MM-DD) a format DD/MM/YYYY
   - `formatRangeDMY`: Formata un rang de dates com a text
   - `formatSimpleDM`: Formata una data com a DD/MM (sense any)
   - `formatDateRanges`: Agrupa dates consecutives en rangs per a una visualització més compacta

2. **Gestió d'Estats d'Assignacions**
   - Gestiona estats com 'Confirmat', 'Pendent', 'No' i 'Mixt'
   - Per a estats 'Mixt', mostra detalls específics de les dates amb cada estat

3. **Generació de Descripcions**
   - Crea descripcions completes i llegibles per a esdeveniments de Google Calendar
   - Inclou tota la informació rellevant de l'esdeveniment i les seves assignacions
   - Formata notes i detalls addicionals de manera consistent

#### Ús típic:

```typescript
import { formatEventFrameForGoogleCalendar } from '../utils/googleCalendarSyncFormatter';

// Dins de la lògica de sincronització:
const eventDescription = formatEventFrameForGoogleCalendar(eventFrame, peopleGroups);

const eventResource = {
  summary: eventFrame.name,
  description: eventDescription,
  location: eventFrame.place || '',
  start: { date: eventFrame.startDate },
  end: { date: addDaysISO(eventFrame.endDate, 1) }
};
```

#### Estructura de la descripció generada:

```
[Període de l'esdeveniment: DD/MM/YYYY - DD/MM/YYYY]

[Notes Generals]
{notes de l'esdeveniment}

[Assignacions]
• Nom de la Persona: DD/MM/YYYY - DD/MM/YYYY (Estat)
  - Notes específiques de l'assignació (si n'hi ha)
  - En cas d'estat Mixt: Detall de les dates amb cada estat
```

### 5.10. Càrrega de Dades Resilient (Migració -> Validació -> Reparació)
