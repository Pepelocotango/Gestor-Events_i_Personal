...
// Versió simplificada per a mòbil: sense format de data ni lògica complexa
const createDefaultTechSheet = (eventFrame: Omit<EventFrame, 'id' | 'assignments' | 'personnelComplete' | 'techSheet'>): TechSheetData => {
    ...
    return {
      ...
      date: new Date(eventFrame.startDate).toLocaleDateString(), // Format simple
      showTime: '',
      showDuration: '',
      technicalProviders: [],
      generalNotes: '',
...
interface EventDataActions {
    ...
    // Simplificat per a mòbil: sense validació, reparació ni Google.
    loadData: (data: AppData | null) => Promise<{ status: 'ok' | 'error'; message?: string, type?: 'success' | 'error' | 'info' | 'warning' }>;
    exportData: () => Promise<AppData>;
    setPersonnelComplete: (eventFrameId: string, complete: boolean) => void;
    setHasUnsavedChanges: (value: boolean) => void;
    // Funcions de Google eliminades per a mòbil
...
export const useEventDataStore = create<EventDataState & EventDataActions>()(
  temporal(
    immer(
      (set, get) => ({
        ...
        setManualExpandedFrameIds: (updater: (prev: Set<string>) => Set<string>) => {
            set(state => ({ manualExpandedFrameIds: updater(state.manualExpandedFrameIds) }));
        },
        toggleEventListExpanded: () => set((state) => ({ isEventListExpanded: !state.isEventListExpanded })),

        // DATA HYDRATION (MOBILE)
        _applyDataToState: (data: AppData) => {
            const loadedEventFrames: EventFrame[] = (data.eventFrames || []).map((efExport: EventFrameForExport) => ({
                ...
                techSheet: efExport.techSheet || createDefaultTechSheet(efExport as EventFrame), // Simplificat
            }));
        ...
        loadData: async (data: AppData | null) => {
            ...
            // Versió simplificada: sense migració, validació ni reparació.
        ...
        exportData: async () => {
            ...
            // Eliminada la part de Google Config
            const dataToExport = { peopleGroups, eventFrames: eventFramesForExport, materialItems, assignments: allAssignmentsList };
            ...
            for (let d = new Date(newStartDate); d <= newEndDate; d.setDate(d.getDate() + 1)) {
                ...
                if (conflictingAssignments.length > 0) {
                    const conflictDetails = conflictingAssignments.map(conflict => {
                        ...
                        return `"${conflictingEvent?.name}" el ${new Date(currentDateStr).toLocaleDateString()}`; // Format simple
                    }).join(", ");
        ...
        if (!force) {
            ...
            const checkDateRange = (start: Date, end: Date, statusToCheck: AssignmentStatus | { [date: string]: AssignmentStatus }) => {
                for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
                    ...
                    if (conflictingAssignments.length > 0) {
                        const conflictDetails = conflictingAssignments.map(conflict => `"${get().eventFrames.find(ef => ef.id === conflict.eventFrameId)?.name}" el ${new Date(currentDateStr).toLocaleDateString()}`).join(", ");
    ...
    // GOOGLE & SYNC (ELIMINAT PER A MÒBIL)
