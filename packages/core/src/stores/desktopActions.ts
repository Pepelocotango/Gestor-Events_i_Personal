...
export const initializeDesktopActions = (adapter: PersistenceAdapter) => {
    persistenceAdapter = adapter;
};

export const refreshGoogleEvents = async (): Promise<{ success: boolean, message?: string, type?: 'success' | 'error' | 'info' | 'warning' }> => {
...
export const executeSync = async (targetCalendarId: string): Promise<any> => {
    ...
    if (persistenceAdapter?.syncWithGoogle) {
        ...
        const enrichedEventFrames = localData.eventFrames.map(frame => ({
            ...frame,
            googleDescription: generateGoogleEventDescription(frame as EventFrame, peopleGroups),
        }));
...
export const syncWithGoogle = async (): Promise<void> => {
